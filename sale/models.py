...
class SaleOrder(generic.BO):
    ...
    partner = models.ForeignKey(Partner, verbose_name=_("partner"), limit_choices_to={"partner_type": "C"}, null=True,
...
class SaleItem(models.Model):
    ...
    订单明细
    """
    master = models.ForeignKey(SaleOrder, null=True, on_delete=models.SET_NULL)
...
class PaymentCollection(generic.BO):
    ...
    so = models.ForeignKey(SaleOrder, verbose_name=_("sale order"), null=True, on_delete=models.SET_NULL)
...
class OfferSheet(generic.BO):
    ...
    attach = models.FileField(_('offer sheet file'), blank=True, null=True, upload_to='offer sheet',
                              help_text='您可导入报价明细，模板请参考文档FD0006')
    status = models.BooleanField(_("executed"), default=0)
    event_time = models.DateTimeField(_("event time"), blank=True, null=True)
...
class OfferItem(models.Model):
    ...
    订单明细
    """
    master = models.ForeignKey(OfferSheet, null=True, on_delete=models.SET_NULL)
    material = models.ForeignKey(Material, verbose_name=_("material"),
                                 limit_choices_to={"is_virtual": "0", 'can_sale': '1'}, blank=True, null=True,
                                 on_delete=models.SET_NULL)
    measure = models.ForeignKey(Measure, verbose_name=_("measure"), blank=True, null=True, on_delete=models.SET_NULL)
